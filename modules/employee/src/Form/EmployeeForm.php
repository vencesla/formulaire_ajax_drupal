...
class EmployeeForm extends FormBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if(!preg_match("/^([a-zA-Z' ]+)$/",$lastName)) {
      $form_state->setErrorByName('emp_lastname', $this->t('Enter the valid name'));
    }

    if(!\Drupal::service('email.validator')->isValid($email)) {
      $form_state->setErrorByName('emp_email', $this->t('Enter the valid email'));
    }

    if(!preg_match("/^\d{1,6}$/",$zipcode)) {
      $form_state->setErrorByName('emp_zipcode', $this->t('Enter the valid zipe code'));
    }
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->messenger()->addStatus($this->t('The employee data has been sent.'));
